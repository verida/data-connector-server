...
describe(`${providerName} Tests`, function() {
    ...
    describe("Fetch API data", () => {
        ...
        it("Can fetch Post data", async () => {
            const syncData = await provider.sync(creds.accessToken, creds.refreshToken)
        ...
        it("Can fetch Following data", async () => {
            ...
            assert.equal(syncData[SCHEMA_FOLLOWING].length, 69, `Correct number of following records received`)
        })
    })
})