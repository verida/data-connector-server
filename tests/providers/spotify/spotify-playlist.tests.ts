  ...
  describe(`Fetch ${providerId} data`, () => {
    it(`Can pass basic tests: ${handlerName}`, async () => {
      ...
      try {
        ...
        const results = <SchemaPlaylist[]>response.results;
        ...
        const secondBatchResults = <SchemaPlaylist[]>secondBatchResponse.results;
    ...
    it(`Should ensure second batch items aren't in the first batch`, async () => {
      ...
      const firstBatchItems = <SchemaPlaylist[]>firstBatchResponse.results;

      const secondBatchResponse = await handler._sync(api, firstBatchResponse.position);
      const secondBatchItems = <SchemaPlaylist[]>secondBatchResponse.results;
