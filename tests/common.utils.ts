...
export default class CommonUtils {
    ...
    static syncConnector = async (provider: string, accessToken: string, refreshToken: string, did: string, nonce: string): Promise<any> => {
        return await axios.get(`${SERVER_URL}/sync/${provider}?accessToken=${accessToken}&refreshToken=${refreshToken}&did=${did}&nonce=${nonce}`)
    }
