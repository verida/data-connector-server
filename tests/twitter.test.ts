...
describe(`${providerName} Tests`, function () {
  ...
  describe("Fetch API data", () => {
    ...
    it("Can fetch Post data", async () => {
      ...
      assert.equal(
        syncData[SCHEMA_POST].length,
        providerConfig.postLimit,
        `Correct number of posts received (${syncData[SCHEMA_POST].length} != ${providerConfig.postLimit})`
      );
    });
    ...
    it("Can fetch Following data", async () => {
      ...
      assert.equal(
        syncData[SCHEMA_FOLLOWING].length,
        providerConfig.followingLimit,
        `Correct number of following records received`
      );
    });
  });
});
