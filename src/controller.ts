...
export default class Controller {
  ...
  static async test1(req: Request, res: Response, next: any) {
    try {
      ...
      for (const f of files) {
        fileOutput = fileOutput + `<pre>${f}</pre>`;
      }
  ...
  public static setWorkingDir() {
    ...
    fs.readdir("/mnt/storage", (err: any, filename: any) =>
  ...
  public static async callback(req: Request, res: Response, next: any) {
    ...
    const redirectUrl = `https://vault.verida.io/connection-success?provider=${providerName}&accessToken=${
      connectionToken.accessToken
    }&refreshToken=${
      connectionToken.refreshToken ? connectionToken.refreshToken : ""
    }`;
  ...
  public static async sync(req: Request, res: Response, next: any) {
    ...
    const syncRequestDatabaseName = EncryptionUtils.hash(
      `${did}-${DATA_SYNC_REQUEST_SCHEMA}`
    );
    const syncRequestDatastore = await context.openDatastore(
    ...
    for (var schemaUri in data) {
      ...
      logger.info(
        `Inserting ${data[schemaUri].length} records into database: ${databaseName} (${info.databaseHash})`
      );
